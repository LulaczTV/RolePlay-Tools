    ...
    public class Ooc : ICommand
    {
        ...
        public bool Execute(ArraySegment<string> arguments, ICommandSender sender, out string response)
        {
...
#endif
            {
                ...
                {
                    Plugin.Instance.hintManager.EnqueueHint(ply, $"<voffset={Plugin.Instance.Config.Voffset}><color=yellow>{player.DisplayNickname}:</color> <color=red>.ooc {text}</color></voffset>");
                }
            }
            response = "Sent!";
            return true;
